 ...
 * Author: soyo
 *
 * Created on 8 June 2017, 2:48 PM
 */

#ifndef __OPENVSWITCH_GTP_H
#define __OPENVSWITCH_GTP_H 1
...
struct gtp0_header {    /*    Descriptions from 3GPP 09.60 */
  ...
  ovs_be16 length;	/* 03 Length (of G-PDU excluding header) */
  ovs_be16 seq;	        /* 05 Sequence Number */
  ovs_be16 flow;	        /* 07 Flow Label ( = 0 for signalling) */
...
struct gtp1_header_long {  /*    Descriptions from 3GPP 29060 */
  uint8_t  flags;         /* 01 bitfield, with typical values */
                           ...
                           /*    .......0 PN: N-PDU Number flag */
  uint8_t  type;	   /* 02 Message type. T-PDU = 0xff */
  ovs_be16 length;	   /* 03 Length (of IP packet or signalling) */
  ovs_be32 tei;	   /* 05 Tunnel Endpoint ID */
  ovs_be16 seq;	   /* 10 Sequence Number */
